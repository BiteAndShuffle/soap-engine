...
describe('T-1 Owner 指定検索ケース（35 件）', () => {
  ...
  const OWNER_CASES: Array<[string, string]> = [
    ...
    ['先発品名', 'ツイミーグ'],
    ['先発品名', 'ジャヌビア'],
    // 2026-09: インタール点眼液は販売中止のため chemical mediator の Owner-approved
    // bridge から非収載となった（O-8）。本ケースが担っていた役割（同 module の先発品名
    // による到達性・manifest parity）は、現行収載の先発品 ゼペリン（点眼液）が
    // そのまま引き継ぐ。カテゴリ構成・総数 35 は変更しない。
    ['先発品名', 'ゼペリン'],
    ['読み仮名', 'りべるさす'],
    ['読み仮名', 'ひるどいど'],
    ['読み仮名', 'ぜぺりん'],
