...
describe('M2: 全 read site が primaryNode の authoritative field を直参照している', () => {
  ...
  test('handleSToggle deps に primaryNode.matchedBrandName / primaryNode.resolvedDrugName が含まれる（Unit 4D-3b: node branch の module 解決用に allModules / moduleData を追加。Q-RAPID1: targetModule は Rapid v2 write guard 用に追加）', () => {
    assert.ok(
      src.includes('}, [primaryNode.matchedBrandName, primaryNode.resolvedDrugName, activeModuleData, allModules, moduleData, confirmDiscard, targetModule])'),
