 ...
 *      主語・内容はそのまま。
 *   ③ 動詞固定列挙（主語が落ちないパターンのみ）
 *      「継続します。」→「継続。」のように動詞を体言止めにする形のみ許可。
 *      「を認めます。」→「を認める。」「を認めず。」など「は/を」が保持される形のみ許可。
 *      「となる。」のように前半が可変なものは体言止め禁止（前半欠損リスク）。
 *
 * 【禁止パターン】（削除した理由）
...
const DENSITY_RULES: ReplacePair[] = [
  ...
  [/を認めます。/g, 'を認める。'],
  [/は認めます。/g, 'は認める。'],
  // 「を認めません。」→「を認めず。」（カルテ文体。は/を が保持されるので主語欠損なし）
  [/を認めません。/g, 'を認めず。'],
  [/は認めません。/g, 'は認めず。'],
  // 「を認めない。」→「を認めず。」（常体も同様にカルテ文体へ）
  [/を認めない。/g, 'を認めず。'],
  [/は認めない。/g, 'は認めず。'],
