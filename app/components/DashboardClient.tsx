...
export default function DashboardClient({ moduleData, allModules }: DashboardClientProps) {
  ...
  // handleNlpGenerate 成功時に true、シナリオ切替 / manual 戻り / NLP 失敗時に false にリセットする。
  ...
  useEffect(() => {
    ...
    if (selectedScenarioId !== null && primaryScenario) {
      ...
      isNlpOriginRef.current = false  // シナリオ切替で NLP 原本フラグをリセット
      ...
      confirmDiscard(() => {
        setSelectedAddonIds(prev => {
          ...
          if (isNlpOriginRef.current) {
            ...
            const fields = (personaEnabled && guard)
              ? applyPersonaToFieldsWithGuard(overlaid, true, selectedPersona, guard)
              : overlaid
            // rawPrimaryFieldsRef は NLP 出力のまま維持（上書きしない）
  ...
  const handleNlpGenerate = useCallback((patientInput: string) => {
    ...
    if (result.soap) {
      ...
      if (result.scenarioId) {
        // setSelectedScenarioId で useEffect([selectedScenarioId]) が走るが、
        // isNlpOriginRef.current が true なのでその中で false にリセットしてはならない。
        // useEffect は uiModeRef.current === 'nlp' ガードで早期リターンするため問題ない。
