...
#     変更していない。
# 12. se_hypo_none の A 文言 → ユーザー最終原稿を正本として確定（2026-07-11）。
#     "SGLT2阻害薬（糖尿病）による低血糖症状は現時点で認められず、治療継続が可能である。"
#     （2026-07-11 の DRAFT 作成時に一時的に別表現へ変更したが、ユーザーより
#     最終原稿と異なる旨の指摘を受け、最終原稿の文言へ差し戻した）。
#     単剤では低血糖が起こりにくいことおよび他の糖尿病薬併用時の低血糖リスクは
#     A ではなく P 側で補足する方針（P 文言「単剤では低血糖は起こりにくい薬
#     ですが、他の糖尿病薬と併用中は低血糖が起こることがあります」に既存の
#     記述として存在。A はシナリオ判定結果の記録に留め、P と役割を重複させない）。
...
服用中は、尿量が増えたり、のどが渇きやすくなることがあります。
...
SGLT2阻害薬（糖尿病）　処方
A
SGLT2阻害薬（糖尿病）による低血糖は現時点で認められず、治療継続が可能である。
P
...
症状が軽く、日常生活に支障がなければ、経過を確認しながら継続できます。
日常生活に支障がある場合はご相談ください。
脱水を防ぐため、こまめな水分補給を心がけてください。
P_CLOSING
次回、治療経過および副作用の有無を確認。
