...
drug:
  ...
  # ─────────────────────────────────────────
  # ブランド名読み「ついみーぐ」・成分名読み「いめぐりみん」はユーザー指定値を
  # そのまま採用する（kataToHira による機械的なひらがな変換のみ。推測なし）。
  #
  # 【SH-1B（DP-21・Owner D-2）による変更（2026-09）】
  #   塩形読み「いめぐりみんえんさんえん」は当初「塩非表記/塩表記の2形式併記」として
  #   併記していたが、検索面から撤去した（塩／水和物の正式名読みは検索面を持たない）。
  #   現在の成分名読みは基本一般名読み「いめぐりみん」のみ。
  #
  ...
  # 成分名読み「いめぐりみん」は module 単位 nameAliases 側のみに存在し
  # brandCatalog.aliases には複製していないため、aliasToBrand の
  # 対象外（既存単剤モジュールと同型の扱い）。
  # 塩形読み「いめぐりみんえんさんえん」は SH-1B（DP-21）で撤去済み。
