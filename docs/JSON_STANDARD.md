...
- **`{{drug_subject}}` の解決経路は 2 系統である（Q-S2 U-4b・2026-08-12 時点）**
  - **検索由来の候補**（`DrugSuggestionItem`）: `BrandResolution`（`lib/brandResolution.ts`）→ `resolveSubjectFromResolution()`（`lib/drugSubject.ts`）が正式な解決経路。表示文字列や brand 名の比較から主語を逆算しない
  - **`BrandResolution` を持たない経路**（Express・legacy 非検索経路）: 引き続き `resolveDrugName()`（`lib/drugSubject.ts`）を使用する
  - `denotation: 'module'`（指示対象が未確定）では主語を確定しない。この状態は SOAP 生成へ進ませない（Q-S2 U-5）
...
**SOAP本文の塩類名表記ルール（S/O/A/P/P_CLOSING）**

`scenarios[].S` / `O` / `A` / `P` および followupProfiles の `P`（P_CLOSING）など、**患者向けに表示されるSOAP本文**では、読みやすさを優先し、塩類名（〜塩酸塩／〜メシル酸塩／〜マレイン酸塩／〜リン酸塩等）を表示しない。有効成分名のみを記載する。
...
正: "○○は、..."（一般的な例）
誤: "○○塩酸塩は、..." / "○○メシル酸塩は、..." / "○○マレイン酸塩は、..."
```

**適用対象**: `S` / `O` / `A` / `P` / `P_CLOSING`（通常の患者向けSOAP本文表示）
...
**運用**: 本ルールは新規作成モジュールに標準適用する。既存モジュールへの一括遡及修正は行わない。個別に気づいた箇所は都度修正する。
