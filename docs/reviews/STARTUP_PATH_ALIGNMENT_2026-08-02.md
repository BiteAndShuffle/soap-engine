...
| **OD-R4-1c** | Lifecycle の読込構成を次で確定する<br>・`docs/DEVELOPMENT_STANDARD.md` §10 前文および §10.1 は **Base 常時読込に残す**<br>・§10.2〜§10.5 は **Lifecycle Overlay による条件付き読込**とする<br>・§11 は **Base 常時読込に残す**<br>・§10.1 は Lifecycle State の定義・原則・正規台帳との優先関係を担う<br>・§10.2〜§10.5 は成立条件・正規台帳・現在値を担う<br>・R6-b-6 は読込境界の再配置後に再開する<br>**本 Decision は OD-R4-1 の「`docs/DEVELOPMENT_STANDARD.md` 全文を Base とする」という内容を改定する** | 2026-08-03 |
...
### 1.2 OD-R4-1c の採用理由・安全性評価・実装境界

**本節は確定内容を持たない。** 確定内容は §1 の表（`OD-R4-1c`）が保持する。本節は、その判断に至った**測定・評価**と、実装時に守る**制約**を記録する。

#### 採用理由

| # | 内容 |
|---|---|
| 1 | §10.1 は絶対的な固定長ではないが、**資産数に対して固定長の原則層として機能し、Base に配置する構造上の安定性を持つ** |
| 2 | §10.1 は資産名・件数・台帳エントリ・実装依存情報を保持しない |
| 3 | 資産が追加された期間にも §10.1 は増加していない |
| 4 | 〔実測〕§10.1 を変更した履歴として、次の commit を git history へのポインタとして記録する。**いずれも現在値・台帳の追加ではない**<br>・`9979fc5` — 原則の新設<br>・`1963c72` — 適用範囲の明確化<br>・`92b3a20` — 原則の追加 |
| 5 | §10.1 には増加実績が存在するため、**判定は Conditional である** |
| 6 | 「§10.1 を今後一切増やさない」という宣言は Repository 内に存在しない |
| 7 | 増加要因は資産数ではなく、規則の解釈・誤読防止・別軸との責務境界である |
| 8 | **§11 を Base に残す根拠は、変更契機標準を条件付き読込にすると、その標準を読むべきかの判定自体に標準の知識が必要となる循環を避けるためである** |

#### 安全性評価

§10 が必要になる作業を、次の 3 類型として記録する。

| 類型 | 内容 |
|---|---|
| **I** | §10 を編集する、または Lifecycle State を明示的に判断・記録・引用する作業 |
| **II** | Persona・Validator・Canonical の判断から、§10 が**二次依存**として必要になる作業 |
| **III** | Lifecycle と無関係に見える作業の途中で、自己表示と正規台帳の関係など、**§10.1 固有の原則**が必要になる作業 |

**類型 III の実例**: R6-b-5 の `docs/feature-glossary.md` レビュー中に、次の §10.1 の規則が作業途中で必要になった。

- 正規台帳が自己表示より優先する
- 未登録の状態表示は状態確定を意味しない

**類型 II の到達性**: 次の各文書が §10 を Lifecycle State の正本として参照している。

- `docs/PERSONA_PROJECT_PRINCIPLE.md`
- `docs/VALIDATOR_STANDARD.md`
- `docs/JSON_STANDARD.md`

#### 実装境界

| # | 内容 |
|---|---|
| 1 | §10 の読込単位を確定した後に、R6-b-6 の変更契機配置を決める |
| 2 | Lifecycle Overlay の**既存発火条件は変更しない** |
| 3 | 変更対象は**読込対象の記述のみ**とする |
| 4 | **新しい補強トリガーは追加しない** |
| 5 | 実装時に未測定事項が判明した場合は、該当 Unit の Execution Plan で確認する |
| 6 | **「品質維持」の意味を、§11 を Base に残す根拠として使用しない** |
| 7 | 「品質維持」の実測射程については `docs/reviews/INFORMATION_ARCHITECTURE_2026-08-03.md` の Decision 2 実装境界を参照する |
...
## 5C. R6-b-6 — 変更契機の Lifecycle 台帳3節への適用

**本節は Owner Decision を保存しない。** 新規の OD-ID は発行しない。本節が記録するのは、承認済み
Execution Plan を Repository へどう反映したかという責務・適用結果・適用範囲の確定・検証結果・
commit 追跡である。
...
| # | 責務 |
|---|---|
| 1 | `docs/DEVELOPMENT_STANDARD.md` §10.2／§10.3／§10.5 へ、§11 変更契機標準に基づく変更契機節を適用した |
| 2 | §10.4（体系移行完了条件）は Owner 判断により対象外とした |

### 5C.2 実施結果

| commit | hash | 対象 | 状態 |
|---|---|---|---|
| **1** | **`5bedd01`** | `docs/DEVELOPMENT_STANDARD.md`（§10.2／§10.3／§10.5 への変更契機節追加） | **完了**（+24 / −0） |
| **2** | 本記録を確定する commit | 本記録（`docs/reviews/STARTUP_PATH_ALIGNMENT_2026-08-02.md` §5C 追記） | **本 commit で完了** |

> **Commit 2 は自身の確定 hash を本文へ記録しない**（§5.1・§5B.2 と同じ理由）。**同定は commit message（`docs(review): record R6-b-6 change trigger rollout`）による。**

### 5C.3 適用範囲の確定

本 Unit の実装過程で、Repository の実測に基づき次の適用範囲を確定した。いずれも新しい成立条件・
Owner 承認条件の追加ではなく、既存規則の解釈および Owner 判断による適用範囲の確定である。

| # | 確定内容 | 根拠 |
|---|---|---|
| 1 | §10.4 は本 Unit の適用対象に含めない | §10.4 は「現在の…資産」形式の正規台帳を持たず、M1〜M8 の成立条件のみを定義する節であるため（Owner 判断） |
| 2 | Future Expansion の正式確定条件は F1〜F5 の充足のみとする | §10.3 本文に §10.2 の L7 に相当する Owner 承認条件は存在しない（実測：grep 0件） |
| 3 | §10.5 経由（Classification Pending からの遷移）の Owner 承認要件は、§10.5 自身の遷移規律にとどめる | §10.3 一般の成立条件（F1〜F5）へは拡張しない |
| 4 | Archived 用の正規台帳は新設しない | Archived への遷移時はファイル内状態表示の更新を扱いとする（正規台帳が存在するのは §10.2／§10.3 の2状態のみ） |

各節の起点・更新対象の具体的な文言は `docs/DEVELOPMENT_STANDARD.md` §10.2／§10.3／§10.5 本文
（commit `5bedd01`）を正本とし、本節では転載しない。

### 5C.4 検証結果〔実測・2026-08-05〕

| # | 検証 | 結果 |
|---|---|---|
| 1 | commit `5bedd01` の変更ファイルが `docs/DEVELOPMENT_STANDARD.md` の1件のみ | **PASS** |
| 2 | diff が §10.2／§10.3／§10.5 の変更契機追加のみ（+24 / −0） | **PASS** |
| 3 | §10.4／§10.1／§10 前文／§11 本体に差分 | **0 件** |
| 4 | 「台帳へ登録したとき」を主起点にした記述 | **0 件**（判断行為を起点、登録は更新対象側） |
| 5 | 「Archived へ登録」という表現 | **0 件** |
| 6 | §10.3 への新規成立条件（F6等）・一般的 Owner 承認要件の追加 | **0 件** |
| 7 | GG-1／GG-3 の Lifecycle State を確定する記述 | **0 件** |
| 8 | 既存差分4件 | **不変** |

### 5C.5 R6-b-6 で行わなかったこと

| # | 事項 |
|---|---|
| 1 | §10.4 への変更契機適用 |
| 2 | `prompts/vNext/STARTUP_PROMPT.md` の変更 |
| 3 | Decision 2・3 の実装 |
| 4 | `prompts/vNext/HANDOFF.md` の drift 修正 |
| 5 | `docs/feature-glossary.md` の変更 |
| 6 | GG-1／GG-3 の Lifecycle State 確定 |
| 7 | bridge STATUS の変更 |
| 8 | R6-c（enforcement） |
| 9 | git author identity の設定 |
| 10 | push |
...
|---|---|
| Commit 1（`5bedd01`） | **完了** |
| Commit 2（本記録を確定する commit） | **本 commit で完了** |
| **R6-b-6** | **完了** |
| R6-c | **未着手** |
| push | **未実施** |
...
| **OD-R4-1c**（Lifecycle の読込構成） | **確定済み・未実装**（§1 ／ 採用理由・安全性評価・実装境界は §1.2） |
